...
long long gormi(int k1, int k2, int k3, int pre) {
  ...
  int l = k1 * 2, r = k1 * 2 + 1, mid = (k2 + k3) >> 1;
  if (pre <= mi[l])
    return 1ll * pre * (mid - k2 + 1) + gormi(r, mid + 1, k3, pre);
...
long long gorma(int k1, int k2, int k3, int pre) {
  ...
  int l = k1 * 2, r = k1 * 2 + 1, mid = (k2 + k3) >> 1;
  if (pre >= ma[l])
    return 1ll * pre * (mid - k2 + 1) + gorma(r, mid + 1, k3, pre);
...
void init(int k1, int k2, int k3) {
  ...
  int mid = k2 + k3 >> 1;
  init(k1 * 2, k2, mid);
  init(k1 * 2 + 1, mid + 1, k3);
  change(k1, k2, k3);
  //	cout<<"ads "<<k1<<" "<<k2<<" "<<k3<<" "<<mi[k1]<<" "<<ma[k1]<<"
  //"<<lmi[k1]<<" "<<rmi[k1]<<" "<<lma[k1]<<" "<<rma[k1]<<endl;
}
void changema(int k1, int k2, int k3, int k4, int k5) {
  ...
  int mid = k2 + k3 >> 1;
  if (mid >= k4)
    changema(k1 * 2, k2, mid, k4, k5);
  else
    changema(k1 * 2 + 1, mid + 1, k3, k4, k5);
  change(k1, k2, k3);
}
void changemi(int k1, int k2, int k3, int k4, int k5) {
  ...
  int mid = k2 + k3 >> 1;
  if (mid >= k4)
    changemi(k1 * 2, k2, mid, k4, k5);
  else
    changemi(k1 * 2 + 1, mid + 1, k3, k4, k5);
  change(k1, k2, k3);
}
int pr = 0;
long long golmi(int k1, int k2, int k3, int k4, int k5) {
  ...
  if (k2 >= k4 && k3 <= k5) {
    long long ans = golmi(k1, k2, k3, pr);
    pr = min(pr, mi[k1]);
    return ans;
  }
  int mid = k2 + k3 >> 1;
  return golmi(k1 * 2 + 1, mid + 1, k3, k4, k5) +
         golmi(k1 * 2, k2, mid, k4, k5);
}
long long golma(int k1, int k2, int k3, int k4, int k5) {
  if (k2 > k5 || k3 < k4) return 0;
  if (k2 >= k4 && k3 <= k5) {
    long long ans = golma(k1, k2, k3, pr);
    pr = max(pr, ma[k1]);
    return ans;
  }
  int mid = k2 + k3 >> 1;
  return golma(k1 * 2 + 1, mid + 1, k3, k4, k5) +
         golma(k1 * 2, k2, mid, k4, k5);
}
long long gormi(int k1, int k2, int k3, int k4, int k5) {
  if (k2 > k5 || k3 < k4) return 0;
  if (k2 >= k4 && k3 <= k5) {
    long long ans = gormi(k1, k2, k3, pr);
    pr = min(pr, mi[k1]);
    return ans;
  }
  int mid = k2 + k3 >> 1;
  return gormi(k1 * 2, k2, mid, k4, k5) +
         gormi(k1 * 2 + 1, mid + 1, k3, k4, k5);
}
long long gorma(int k1, int k2, int k3, int k4, int k5) {
  if (k2 > k5 || k3 < k4) return 0;
  if (k2 >= k4 && k3 <= k5) {
    long long ans = gorma(k1, k2, k3, pr);
    pr = max(pr, ma[k1]);
    return ans;
  }
  int mid = k2 + k3 >> 1;
  return gorma(k1 * 2, k2, mid, k4, k5) +
         gorma(k1 * 2 + 1, mid + 1, k3, k4, k5);
}
map<int, int> M[110000];
void solve() {
  scanf("%d%d%d%d", &n, &m, &K, &q);
  len = 0;
  assert(1 <= n && n <= 100000 && 1 <= m && m <= 100000 && 1 <= K &&
         K <= 100000 && 1 <= q && q <= 100000);
  for (int i = 1; i <= m; i++)
    S[i].clear(), S[i].insert(0), S[i].insert(n + 1), M[i].clear();
  for (int i = 1; i <= K; i++) {
    len++;
  ...
  for (int nowx = 1; nowx <= n; nowx++) {
    ...
    for (int i = pre; i < now; i++)
      if (x[i].pd) {
        ...
        pr = n + 1;
        long long ans = golmi(1, 1, m, 1, x[i].y);  // cout<<ans<<" ";
        pr = n + 1;
        ans += gormi(1, 1, m, x[i].y, m);  // cout<<ans<<" ";
        pr = 0;
        ans -= golma(1, 1, m, 1, x[i].y);  // cout<<ans<<" ";
        pr = 0;
        ans -= gorma(1, 1, m, x[i].y, m);  // a cout<<ans<<endl;
        set<int>::iterator k = S[x[i].y].lower_bound(x[i].x);
        int r = (*k);
        k--;
        int l = (*k);
        ans -= (r - l);
        k = SS.lower_bound(x[i].y);
        r = (*k);
        k--;
        l = (*k);
        ans -= (r - l - 1);
